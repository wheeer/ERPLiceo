  ...
  generarLiquidaciones() {
    ...
    this.http.post<any>(
      ...
      next: (response) => {
        ...
        this.cargarRemuneraciones();
        setTimeout(() => {
          this.payrollData.forEach((payroll, index) => {
            setTimeout(() => this.descargarPDF(payroll), index * 800);
          });
        }, 1500);
      },
      error: (error) => {
        ...
        if (error.status === 400 && error.error?.message?.includes('Ya existen')) {
          this.toastService.show('Ya existen liquidaciones para este período. Descargando PDFs...', 'info');
          this.payrollData.forEach((payroll, index) => {
            setTimeout(() => this.descargarPDF(payroll), index * 800);
          });
        } else {
